...
class TopicsController extends Controller
{
    ...
    private $thumbPath = "uploads/thumbs/";
    ...
    public function index($webmasterId)
    {
        ...
        if (count($WebmasterSection) > 0) {
            ...
            if (@Auth::user()->permissionsGroup->view_status) {
                $Topics = Topic::where('created_by', '=', Auth::user()->id)->where('webmaster_id', '=',$webmasterId)->orderby('row_no','desc')->paginate(env('BACKEND_PAGINATION'));
            } else {
                $Topics = Topic::where('webmaster_id', '=', $webmasterId)->orderby('row_no','desc')->paginate(env('BACKEND_PAGINATION'));
            }
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        if (count($WebmasterSection) > 0) {
            ...
            if ($request->$formFileName != "") {
                ...
                $path = $this->getUploadPath().$fileFinalName;
                // $request->file($formFileName)->move($path, $fileFinalName);


                //Thumbnail
                
                $originalImage= $request->file($formFileName);
                
                //Resize image here
                $thumbnailpath = $this->getThumbPath().$fileFinalName;

                $img = Image::make($originalImage);

                $img->save($path);

                $img->resize(371, 221, function($constraint) {
            ...
            $Topic->seo_url_slug_vi = $slugs['slug_vi'].'-'.rand(1111,9999);
            $Topic->seo_url_slug_en = $slugs['slug_en'].'-'.rand(1111,9999);
